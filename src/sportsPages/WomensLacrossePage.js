...
function WomensLacrossePage({ signOut, toggleDropdown, dropdownOpen }) {
  ...
  useEffect(() => {
    const fetchMatches = async () => {
      try {
        const matchData = await client.graphql({ query: listSportingEvents, variables: { filter: { Sport: { eq: "WLA" } } } });
        ...
        {matches.length === 0 ? (
          ...
          matches.map((match, index) => (
            <div key={match.id} className="betting-line-item betting-line">
              <Image src={mensLacrosseLogo} className="sport-logo" alt="Women's Lacrosse Logo" width="50px" height="50px" />
...
export default WomensLacrossePage;  // Updated export name
