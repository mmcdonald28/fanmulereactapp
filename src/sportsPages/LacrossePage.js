...
function MensLacrossePage({ signOut, toggleDropdown, dropdownOpen }) {
  ...
  useEffect(() => {
    const fetchMatches = async () => {
      try {
        const matchData = await client.graphql({ query: listSportingEvents, variables: { filter: { Sport: { eq: "MLA" } } } });
        ...
        {matches.length === 0 ? (
          ...
          matches.map((match, index) => (
            <div key={match.id} className="betting-line-item betting-line">
              <Image src={mensLacrosseLogo} className="sport-logo" alt="Men's Lacrosse Logo" width="50px" height="50px" />
